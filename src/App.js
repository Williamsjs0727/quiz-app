...
export default function App() {  
  ...
  async function ensureStartTime(pid) {  
    if (!pid || data.quizStatus !== "running" || !currentQuestion) return;  
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">  
        ...
        <div className="flex items-center gap-2">  
          {mode === "admin" ? (  
            ...
            <>  
              {user && <div className="hidden sm:block text-sm text-slate-300">{user.userName}</div>}  
              <button  
                onClick={onOpenAdmin}  
                className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-1.5 text-sm text-slate-200 hover:bg-slate-800"  
              >  
...
function ParticipantRouter({  
  ...
  if (view.type === "auth") {  
    return <AuthPage onRegister={onRegister} onRecover={onRecover} />;  
  }  
